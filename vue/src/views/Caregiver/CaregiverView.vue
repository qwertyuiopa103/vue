<template>
  <div>
    <v-data-table :loading="loading" :headers="headers" :items="caregivers" :search="search" class="elevation-1">
      <template v-slot:top>
        <v-toolbar flat>
      ...
      <template v-slot:item.user.userPhoto="{ item }">
        <v-avatar size="40">
          <img v-if="item.user && item.user.userPhoto" :src="item.user.userPhoto" alt="使用者照片">
        ...
        <v-card-text>
          <v-container>
            <v-row>
              <!-- 顯示但不可編輯的欄位 -->
              <v-col cols="6">
                <v-text-field :value="editedItem.caregiverNO" label="護工編號" disabled></v-text-field>
              </v-col>

              <!-- 可編輯的欄位 -->
              <v-col cols="6">
                <v-select v-model="editedItem.caregiverGender" :items="['男', '女']" label="性別" required></v-select>
              </v-col>

              <v-col cols="6">
                <v-text-field v-model.number="editedItem.caregiverAge" label="年齡" type="number" required></v-text-field>
              </v-col>

              <v-col cols="6">
                <v-text-field v-model.number="editedItem.expYears" label="工作經驗(年)" type="number"
                  required></v-text-field>
              </v-col>

              <v-col cols="6">
                <v-text-field v-model.number="editedItem.hourlyRate" label="時薪" type="number" prefix="$"
                  required></v-text-field>
              </v-col>

              <v-col cols="12">
                <v-textarea v-model="editedItem.eduExperience" label="學歷與經驗" rows="3"></v-textarea>
              </v-col>
            </v-row>
          </v-container>
        </v-card-text>
...
export default {
  ...
  methods: {
    addItem() {
      this.$router.push('/admin/caregiver/insert');
    },
    close() {
      this.dialog = false;
      this.$nextTick(() => {
    ...
    async deleteItem(item) {
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`http://localhost:8080/api/CaregiverAdmin/DeleteCaregiver?caregiverNO=${item.caregiverNO}`);
