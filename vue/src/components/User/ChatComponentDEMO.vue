...
async function sendMessage() {
    ...
    // 如果使用者發送的是問題一或問題二，使用預設答案
    if (
        messageText === '我家長輩生重病需要看護，地點在台北和新北' ||
        messageText === '我家長輩只需要基本照護，有推薦的嗎?'
    ) {
        setTimeout(() => {
            // 刪除「搜尋中…」訊息
            ...
            if (messageText === '我家長輩生重病需要看護，地點在台北和新北') {
                messages.value.push({
                    role: 'assistant',
                    content: `以下是為您找到的看護資訊：
                    選項1 姓名: 傅豔, 階級: 專業護理師, 地區: 台北, 新北, 桃園, 年齡: 42, 性別: 男
                    選項2 姓名: 黎元元, 階級: 專業護理師, 地區: 台北, 年齡: 51, 性別: 女
                    若需要更多資訊，請再告知我！`,
                    timestamp: getFormattedTime()
                });
            } else if (messageText === '我家長輩只需要基本照護，有推薦的嗎?') {
                messages.value.push({
                    role: 'assistant',
                    content: `以下是為您找到的看護資訊：
                            選項1 姓名: 巫芬辰, 階級: 初階看護人員, 地區: 台北, 新北, 年齡: 28, 性別: 女
                            選項2 姓名: 游曼藍, 階級: 初階看護人員, 地區: 高雄, 年齡: 29, 性別: 女
                            若需要更多資訊，請再告知我！`,
                    timestamp: getFormattedTime()
                });
            }
